<?php /* Smarty version 2.6.19, created on 2012-08-28 10:29:17
         compiled from ni/nidashboard/opsMailer/index.tpl */ ?>
<html>
    <head>
    ...
    function getCountry(){
        ...
        var countryEle = document.getElementById("cntryID");
        var selCityEle = document.getElementById("selectedCityID");
        if(countryEle.options[countryEle.selectedIndex].value == "select"){
            cityEle.style.display = "none";
            return false;
        }
        var count = 0;
        cityEle.length = 0;
        ...
        for(i=1;i<countryEle.length;i++){
            if(countryEle[i].selected){
                arr = countryEle[i].value.split("|X|");
                cntryVal = arr[0];
                cityArr = arr[1].split(",");
                for(j=0;j<cityArr.length;j++){
                 opt = new Option();
                slct = cityArr[j].split("#");
                opt.text = countryEle[i].text+"-"+slct[1];    
                opt.value = cntryVal+"."+slct[0];  
                if(selCityEle.value.indexOf(opt.value) != -1)
                opt.selected = true; 
                cityEle[count++] = opt; 
                }    
            } 
        }
        cityEle.style.display = "";
...
/nis_statusView.php" target="_blank">Status View</a>
 ...
 <?php endif; ?></span>
            ...
            <select name="cntry[]" id="cntryID" size="5" multiple="multiple" class="w225" onChange="getCountry();">
...
><?php echo $this->_tpl_vars['COUNTRY_ARR'][$this->_sections['i']['index']]['label']; ?>
          ...
          <select name="city[]" id="cityID" size="5" multiple="multiple" class="w225" style="display:none">
            </select>
...
"/>
 ...
 <?php endif; ?></span>
        ...
        <input type="hidden" name="selectedCity" id="selectedCityID" value="<?php echo $this->_tpl_vars['CITY_ARR']; ?>
">
        </form>
