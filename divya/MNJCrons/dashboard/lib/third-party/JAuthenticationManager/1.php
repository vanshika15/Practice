...
class Authenticator
{
  ...
  public function forceAuthentication()
  {
    ...
    switch($status ){
      ...
      case "REMEMBERED"      : $this->redirectToAuthServer(2);
      case "TIMEDOUT"        : $this->redirectToAuthServer(3);
      case "NOTAUTHENTICATED": $this->redirectToAuthServer(0);
    }
  }
  ...
  */
    ...
    if ( $PlainText )
    {
      ...
      if ( $current_time - $Tout < $_TOUT ) {
        ...
        if ( strtoupper($flagRemember) == 'Y' )
          $cookie_expiry = $newTime + 3888000;	// Cookie expiry set to 45 days
        else
          $cookie_expiry = 0;
        ...
        if($CookieVars["RS"] == "1")
          setcookie("MYNAUKRI[RS]", 1, $cookie_expiry, "/", _COOKIE_DOMAIN , false, true); /// For Resume Services cookie

        return "AUTHENTICATED";
      } else if (strtoupper($flagRemember) == 'Y' && $current_time - $Tout < $_TOUT_REMEMBER_ME ){
        return "REMEMBERED";
      }
    }
    return "TIMEDOUT";
  ...
  private function autoLoginUser($userName)
  {
    $ndb = $this->getDatabaseConnection();
    ...
    else
    {
      ...
      $sqlConnect = "INSERT INTO resman5.CONNECT(ID, RESID, USERNAME, IPADDR, TIME) VALUE(\"\", \"".$resid."\", \"".addslashes($userName)."\", \"$ipaddr\", \"".$CurTime."\")";
