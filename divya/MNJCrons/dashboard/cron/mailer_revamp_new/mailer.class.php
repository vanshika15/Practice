		...
		foreach($arrChunkUserList as $key => $arrUserList){
			...
			foreach($arrUserList as $arrDetails) {
				...
				if($arrDetails['EMAIL']!='' && $validData) {
					...
					/** sending mail */
	//				$email = ($this->env == 'LIVE') ? 'sankalp_niec@yahoo.in' : $this->reportingEmail;
	//echo "zxxxxxxxxxxxxxxxxxxxx".$email."\n";die;
					...
					//if(!$this->sendMail($email,$this->subject)) {
						...
						/** mail have been sent.Now call to push_user function so that this can be recorded */
						if($push_user)
...
echo $arrDetails['EMAIL']."===".$this->subject."++".$arrDetails['username']."\n";
                                if(!$this->sendMail($email,$this->subject)) {
					try {
                                               if ($this->MID == 15) {
						       echo $cmd = "/usr/local/php/bin/php /usr/local/apache/htdocs/dashboard/cron/mailer_revamp/NaukriMQ/RVMPush.php "."\"{$arrDetails['username']}\"";
						       echo "\n";
						       passthru($cmd);
					       }
                                        } catch (Exception $e) {
	...
	protected function getProfileDetail($username)
	{
		...
		while(($res = execute_query($sql, $this->log_conn)) === false) $this->LogErrorAndReconnect("getProfileDetail fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
		...
		foreach ($ID as $cityId) {
			if($cityId >= 9999) {
				$sql = "SELECT LABEL FROM resman5.MCITY WHERE ID =".$cityId;
				 while(($res = mysql_query($sql, $this->log_conn)) === false) $this->LogErrorAndReconnect("getLocationFromMap fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
				while($row = mysql_fetch_assoc($res))
	...
	function getPPGCourseFromMap($ppgCourseId) {
		...
		if($ppgCourseId>=9999) {
			...
			while(($res = mysql_query($sql, $this->log_conn)) === false) 
				$this->LogErrorAndReconnect("getPPGCourseFromMap fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
			while($row = mysql_fetch_assoc($res))
	...
	function getPGCourseFromMap($pgCourseId) {
		...
		if($pgCourseId>=9999) {
			$sql = "SELECT PGCOURSE FROM resman5.MPGCOURSE WHERE CID =".$pgCourseId;
			 while(($res = mysql_query($sql, $this->log_conn)) === false) $this->LogErrorAndReconnect("getPGCourseFromMap fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
			while($row = mysql_fetch_assoc($res))
				return $row['PGCOURSE'];
		}
	...
	function getUGCourseFromMap($ugCourseId) {
		...
		if($ugCourseId>=9999) {
			$sql = "SELECT UGCOURSE FROM resman5.MUGCOURSE WHERE CID =".$ugCourseId;
			 while(($res = mysql_query($sql, $this->log_conn)) === false) $this->LogErrorAndReconnect("getUGCourseFromMap fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
			while($row = mysql_fetch_assoc($res))
				return $row['UGCOURSE'];
		}
    ...
    function getUGSpecFromMap($ugSpecId) {
        ...
        if($ugSpecId>=9999) {
            $sql = "SELECT UGSPEC FROM resman5.MUGSPEC WHERE CID =".$ugSpecId;
             while(($res = mysql_query($sql, $this->log_conn)) === false) $this->LogErrorAndReconnect("getUGSpecFromMap fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
            while($row = mysql_fetch_assoc($res))
                return $row['UGSPEC'];
        }
    ...
    function getUGInstFromMap($ugInstId) {
        ...
        if($ugInstId>=9999) {
            $sql = "SELECT INSTITUTE FROM resman5.MUGINST WHERE ID =".$ugInstId;
             while(($res = mysql_query($sql, $this->log_conn)) === false) $this->LogErrorAndReconnect("getUGInstFromMap fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
            while($row = mysql_fetch_assoc($res))
	...
	function getFArea($farea)
	{
		$sql = "SELECT f.LABEL AS FAREA_LABEL FROM resman5.FAREA AS f WHERE f.FAREA_ID = '$farea'";
		 while(($res = mysql_query($sql, $this->log_conn)) === false) $this->LogErrorAndReconnect("getFArea fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
		$row = mysql_fetch_assoc($res);
	...
	function getRole($fid,$rid)
	{
		...
		$sql = "SELECT IF($rid = '-1' OR $rid = '', 'Not Mentioned', r.ROLE) AS ROLE FROM resman5.ROLE AS r WHERE  r.FID = '$fid' AND IF($rid = '-1' OR $rid = '', 1, $rid) = r.RID";
		 while(($res = mysql_query($sql, $this->log_conn)) === false) $this->LogErrorAndReconnect("getRole fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
		$row = mysql_fetch_assoc($res);
		return $row["ROLE"];
	}
	...
	function getIndType($value)
	{
		$sql = "SELECT i.LABEL AS IND_TYPE FROM resman5.INDTYPE AS i WHERE i.VALUE = '$value'";
		 while(($res = mysql_query($sql, $this->log_conn)) === false) $this->LogErrorAndReconnect("getIndType fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
		$row = mysql_fetch_assoc($res);
		return $row["IND_TYPE"];
	}
	...
	function getPrefLocation($ID=array())
	{
		...
		$sql = "SELECT LABEL FROM resman5.DTARGETCITY WHERE ID IN ('$id')";
		 while(($res = mysql_query($sql, $this->log_conn)) === false) $this->LogErrorAndReconnect("getPrefLocation fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
		while($row = mysql_fetch_assoc($res))
			array_push($result,$row['LABEL']);
		return implode(',',$result);
	}
	...
	function getLocation($ID=array(), $checkForOtherCities = false)
	{
		...
		$sql = "SELECT LABEL FROM resman5.DCITY WHERE ID IN ('$id')";
		 while(($res = mysql_query($sql, $this->log_conn)) === false) $this->LogErrorAndReconnect("getLocation fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
		if(mysql_numrows($res) == 0 && $checkForOtherCities) {
			unset($res);
			 $sql = "SELECT LABEL FROM resman5.MCITY WHERE ID IN ('$id')";
	                 while(($res = mysql_query($sql, $this->log_conn)) === false) $this->LogErrorAndReconnect("getLocation fatal mcity".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
		}
		while($row = mysql_fetch_assoc($res))
			array_push($result,$row['LABEL']);
		return implode(',',$result);
	...
	private function isLatestAccount($email, $registrationDate)
	{
		...
		while(($res = execute_query($sql, $this->log_conn)) === false) $this->LogErrorAndReconnect("isLatestAccount fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
		$row = mysql_fetch_assoc($res);
		...
		foreach($this->totalMailerCount as $mid => $count) {
                        if ( $conn ) {
                                ...
                                if (!mysql_num_rows($res)) {
                                        $sql = "INSERT INTO db1.MAILER_TRACK(DATE, MID, SENT_COUNT, OPEN_COUNT) VALUES(CURRENT_DATE, '$mid', '$count', '0')";                   } else {
	...
	protected function checkCV($resID) {
		...
		$res1 = execute_query($sql1, $this->log_conn);
	...
	protected function checkEduNExpDetail($resID,$totalExp) {
		...
		while(($res2 = execute_query($sql2, $this->log_conn)) === false) $this->LogErrorAndReconnect("checkEduNExpDetail fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
...
//		} else
		...
		while(($res3 = execute_query($sql3, $this->log_conn)) === false) $this->LogErrorAndReconnect("checkEduNExpDetail1 fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
		...
		while(($res4 = execute_query($sql4, $this->log_conn)) === false) $this->LogErrorAndReconnect("checkEduNExpDetail2 fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
	...
	protected function checkCTCError($totalExp,$CTC,$CTC_TYPE) {
		...
		if($totalExp >0  && $totalExp < 99 &&( (($expYears >= 1 && $expYears < 99 ) &&  ($ctcLacs <= 0 && $ctcThousands <= 0 )) ||( ($ctcToExperienceRatio < 0.15 || $ctcToExperienceRatio > 5) )) ) {
			return true ;
        } else if($CTC_TYPE == 'U')
            return true;
        else
	...
	protected function checkLocation($Location) {
		...
		while(($resCity = execute_query($sqlCity, $this->log_conn)) === false) $this->LogErrorAndReconnect("checkLocation fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
	...
	protected function checkPhotoStatus($resid)
	{
		...
		while(($res = execute_query($sql, $this->log_conn)) === false) $this->LogErrorAndReconnect("checkPhotoStatus fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
		$row = mysql_fetch_assoc($res);
	...
	protected function checkItSkills($Farea, $Resid)
	{
		if (in_array ($Farea, $this->ID_IT_FAREA)){
			...
			while(($res = execute_query($sql, $this->log_conn)) === false) $this->LogErrorAndReconnect("checkItSkills fatal ".mysql_error($this->log_conn),$this->log_conn,$SERVER_RESUMESLAVE);
			$row = mysql_fetch_assoc($res); 
			if($row[c] > 0)
				return false;
			else
	...
	private function setMetaData($userid, $email, $subject,$mailHTMLContent, $from, $fromName, $mailerId = null) {
            $data['app_id'] = '105';
            $data["job_name"] = "mnjMail";
            $data['meta_data']['recipients'][0]['send_to'] = $email;
            $data['meta_data']['recipients'][0]['userid'] = $userid;
            $data['meta_data']['recipients'][0]['mailSettingCheckProperty'] = 'profile_incomplete_alert';
