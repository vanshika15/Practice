...
class RecruiterViewMailerCleanUpWeekend extends Mailer {
 ...
 private function connectToCvCrm()
        {
         ...
         //       $con = mysql_connect("10.208.67.208:3306","mynauk","mynaukKm7Iv80l",true) or $this->LogError("Connection Error --> ".mysql_error($con),1);
...
$this->conn = $this->connectToCvCrm();
		...
		$sql = "truncate db1.temp_username";
...
$this->conn = $this->connectToCvCrm();
		$result = mysql_query($sql,$this->conn) or $this->LogError($sql." --> ".mysql_error($this->conn)." --> Error number ".mysql_errno($this->conn));
		$sql = "truncate db1.temp_username3";
$this->log_conn = $this->connectToSlave();
