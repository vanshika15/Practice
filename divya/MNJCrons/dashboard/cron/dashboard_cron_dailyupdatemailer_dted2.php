...
function send_mail($date, $str)
{
        ...
        mail("jobseeker.server@naukri.com", "Daily FMark Mailer 2 for $date",$str);
}
...
$msg = "\ndashboard_cron_dailyupdatemailer_dted2.php script started at: " . $dtStart ."\n" ;
...
$smarty = new Smarty;
$smarty->assign("UNSUBSCRIBE_URL", "http://login.naukri.com/nLogin/Login.php?URL=".rawurlencode("http://my.naukri.com/customised/unsubscribe/unsubscribe.php?mailer_id=&mail_cat=UP2&mail_type=ucm"));
...
$currday = $currday -1;
$currday= strftime("%Y-%m-%d",mktime(0,0,0,date("m")-1, date("d")-1, date("Y")));

//$curdate = strftime("%Y-%m-%d",mktime(0,0,0, date("m")-3 , date("d")+5, date("Y")));


$curdate = strftime("%Y-%m-%d",mktime(0,0,0, date("m"), date("d")-15, date("Y")));
...
//$email = "mittal21cen@gmail.com";
	if(checkemail($email)) {
		if (isLatestEntry($dbslave, $myresult["USERNAME"], $myresult["EMAIL"], $myresult["MOD_DT"])) {
			preview($myresult);
...
$dbmaster1 = mysql_connect("resman1.resdex.com:3316", "dashboard","dashboardKm7Iv80l") or die("not connected to master jj".mysql_error()." ff") ;
